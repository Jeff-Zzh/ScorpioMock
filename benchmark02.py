...
l = 4 * 1024 * 1024  # 4 MB 消息长度 bit
model_hyper_param_l = 512 * 1024  # 512 KB 模型超参数 bit
end_edge_distance = 20  # 终端设备与边缘节点之间的距离（米）
edge_cloud_distance = 2000000  # 边缘节点与云服务器之间的距离（米）
N = 10  # 边缘节点连接的终端设备数量
M = 4  # 云节点连接的边缘节点数量
T_idle = 10  # 空闲状态持续时间（秒）
T_sleep = 25  # 睡眠状态持续时间（秒）
T_wake = 1  # 唤醒状态持续时间（秒）

l_infer = 1 * 1024 * 1024  # 1 MB 模型inference需要处理的数据 bit
l_pred = 100 * 1024 * 8  # 100 KB 预测数据量 bit
l_real = 512 * 1024 * 8  # 512 KB 真实数据量 bit
l_model = 20 * 1024 * 1024  # 20 MB 模型训练数据量 bit
