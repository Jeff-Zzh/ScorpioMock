...
class MPDLinear_SOTA(nn.Module):
    """
    ...
    def __init__(self, configs:ModelConfig):
        super(MPDLinear_SOTA, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.series_weights_vector is None: # 避免每次 forward 调用时重新初始化
            ...
            self.series_weights_vector = nn.Parameter(torch.ones(series_num))  # series_num 个 多元分解序列
        ...
        res_trend_season_periodic_output, res_feature_att = self.feature_attention(res_trend_season_periodic_output.permute(0, 2, 1))
        res_trend_season_periodic_output, res_temporal_att = self.pred_len_temporal_attention(res_trend_season_periodic_output.permute(0, 2, 1))

        nonlinear_trend_output, nonlinear_feature_att = self.feature_attention(nonlinear_trend_output.permute(0, 2, 1))
        nonlinear_trend_output, nonlinear_temporal_att = self.pred_len_temporal_attention(nonlinear_trend_output.permute(0, 2, 1))
...
'''
这个模型的核心思想是将时间序列分解为多个趋势成分，然后分别对这些成分进行预测并进行加权求和，bagging生效。
通过这种方式，模型能够更好地捕捉时间序列中的不同模式，从而提高预测的准确性。
此外，通过 individual 参数，模型能够灵活地在每个通道应用单独的线性模型或共享一个线性模型，适应不同的需求
'''