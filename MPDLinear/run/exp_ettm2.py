...
# 定义不同的 seq_len 值列表(间隔为15min)
seq_len_list = [1, 2, 4, 6, 8, 12, 24, 48, 96, 192]

# 配置
config = ModelConfig() # 初始化模型参数配置
dataset_path = '../datasets/9_LTSF_dataset/ori/ETTm2.csv'
...
expand_time_feature_dataset = feature_engineering_for_ettm2_and_save(ori_dataset)
...
selected_features = dataset_exp.columns[:16] # 选前16列作为feature
...
# 保存标准化后的数据到 CSV 文件
standardized_data.to_csv('../datasets/9_LTSF_dataset/processed/standardized/ETTm2_processed_standardized.csv', index=False)
...
# 保存归一化后的数据到 CSV 文件
normalized_data.to_csv('../datasets/9_LTSF_dataset/processed/normalized/ETTm2_processed_normalized.csv', index=False)
...
config.seq_len = None #  [1, 2, 4, 6, 8, 12, 24, 48, 96, 192]
# pred_len选择
...
config.enc_in = len(selected_features) # 特征列数 = 通道数 = 16
