...
# 特征工程
avg_data_list, feature_expand_data_list = feature_engineering(geo_dataset_list)
avg_data_beijing = avg_data_list[0]
avg_data_shanghai = avg_data_list[1]
avg_data_chongqing = avg_data_list[2]
feature_expand_data_beijing = feature_expand_data_list[0]
feature_expand_data_shanghai = feature_expand_data_list[1]
feature_expand_data_chongqing = feature_expand_data_list[2]
...
y = dataset_exp['temperature']  # 目标变量
...
y_label_standardized_df = pd.DataFrame(y_standardized, columns=['temperature'])
# 合并特征feature和目标变量target
standardized_data = pd.concat([X_feature_standardized_df, y_label_standardized_df], axis=1)
# 保存标准化后的数据到 CSV 文件
standardized_data.to_csv('../datasets/weather/weather1_by_day/divide_by_city/standardized/beijing_standardized_weather.csv', index=False)
...
y_label_normalized_df = pd.DataFrame(y_normalized, columns=['temperature'])
# 合并特征和目标变量
normalized_data = pd.concat([X_feature_normalized_df, y_label_normalized_df], axis=1)
# 保存归一化后的数据到 CSV 文件
normalized_data.to_csv('../datasets/weather/weather1_by_day/divide_by_city/normalized/beijing_normalized_weather.csv', index=False)
...
plt.title('Test dataset: Predictions vs True Values')
plt.legend()
plt.grid(True)
