...
from MPDLinear.data_process.data_processor import preprocessing_data, load_data, clean_weather_and_save, \
    ...
    feature_engineering_for_etth1_and_save, feature_engineering_for_ettm2_and_save, \
    feature_engineering_for_traffic_and_save, feature_engineering_for_weather_and_save
...
# 定义不同的 seq_len 值列表(间隔为10 min)
seq_len_list = [1, 3, 6, 9, 12, 60, 120, 240, 360, 480, 540, 720]

# 配置
config = ModelConfig() # 初始化模型参数配置
dataset_path = '../datasets/9_LTSF_dataset/ori/weather.csv'
...
expand_time_feature_dataset = feature_engineering_for_weather_and_save(ori_dataset)
...
selected_features = dataset_exp.columns[:30] # 选前869列作为feature
...
# 保存标准化后的数据到 CSV 文件
standardized_data.to_csv(f'../datasets/9_LTSF_dataset/processed/standardized/{dataset_name}_processed_standardized.csv', index=False)
...
# 保存归一化后的数据到 CSV 文件
normalized_data.to_csv(f'../datasets/9_LTSF_dataset/processed/normalized/{dataset_name}_processed_normalized.csv', index=False)
...
config.seq_len = None # [1, 3, 6, 9, 12, 60, 120, 240, 360, 480, 540, 720]
# pred_len选择
...
config.enc_in = len(selected_features) # 特征列数 = 通道数 = 30
